...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    let refresh1=refresh.refreshToken
    res.cookie("refresh_token", refresh1,{httpOnly:true,sameSite:"None",secure:true,maxAge:1000 * 60 * 60 * 24});
...
router.get('/refresh_token',(req,res)=>{
  try {
      ...
      jwt.verify(refreshToken,process.env.REFRESH_TOKEN_SECRET,(error,user)=>{
          ...
          res.cookie('refresh_token',refreshTk.refreshToken,{httpOnly:true,sameSite:"None",secure:true,maxAge:1000 * 60 * 60 * 24});
          res.json({...token});
      })
  } catch (error) {
