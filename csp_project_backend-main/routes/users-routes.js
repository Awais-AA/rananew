...
router.get('/current-user',authenticateToken, async (req, res) => {
  try {
    ...
    const user = await pool.query('select * from usersdata where user_id = $1',[req.user.user_id])
    const userType =await pool.query('SELECT user_type_name FROM user_type WHERE user_id = $1',[req.user.user_id])
    ...
    res.json({getUserType,userName,userEmail});
  } catch (error) {
    res.status(500).json({error: error.message});
  }
});
